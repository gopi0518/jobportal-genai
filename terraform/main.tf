...
provider "confluent" {
  ...
  kafka_api_key       = confluent_api_key.gdapp-manager-kafka-api-key.id
  kafka_api_secret    = confluent_api_key.gdapp-manager-kafka-api-key.secret
}
...
resource "confluent_service_account" "gdapp-manager" {
  display_name = "gdapp-manager"
  description  = "Service account to manage 'demo' Kafka cluster"
}

resource "confluent_role_binding" "gdapp-manager-kafka-cluster-admin" {
  principal   = "User:${confluent_service_account.gdapp-manager.id}"
  role_name   = "CloudClusterAdmin"
  crn_pattern = confluent_kafka_cluster.basic.rbac_crn
}

resource "confluent_api_key" "gdapp-manager-kafka-api-key" {
  display_name = "gdapp-manager-kafka-api-key"
  description  = "Kafka API Key that is owned by 'gdapp-manager' service account"
  owner {
    id          = confluent_service_account.gdapp-manager.id
    api_version = confluent_service_account.gdapp-manager.api_version
    kind        = confluent_service_account.gdapp-manager.kind
  }
  ...
  # The goal is to ensure that confluent_role_binding.gdapp-manager-kafka-cluster-admin is created before
  # confluent_api_key.gdapp-manager-kafka-api-key is used to create instances of
  # confluent_kafka_topic, confluent_kafka_acl resources.

  # 'depends_on' meta-argument is specified in confluent_api_key.gdapp-manager-kafka-api-key to avoid having
  # multiple copies of this definition in the configuration which would happen if we specify it in
  # confluent_kafka_topic, confluent_kafka_acl resources instead.
  depends_on = [
    confluent_role_binding.gdapp-manager-kafka-cluster-admin
  ]
}
...
resource "confluent_kafka_topic" "click_stream" {
  ...
  partitions_count = 1
  credentials {
    key    = confluent_api_key.gdapp-manager-kafka-api-key.id
    secret = confluent_api_key.gdapp-manager-kafka-api-key.secret
  }
}

# Create a service account for ksqlDB
resource "confluent_service_account" "gdapp-ksql" {
  display_name = "gdapp-ksql"
  description  = "Service account to manage 'demo-ksql' ksqlDB cluster"
}

resource "confluent_role_binding" "gdapp-ksql-kafka-cluster-admin" {
  principal   = "User:${confluent_service_account.gdapp-ksql.id}"
  role_name   = "CloudClusterAdmin"
  crn_pattern = confluent_kafka_cluster.basic.rbac_crn
}

resource "confluent_role_binding" "gdapp-ksql-schema-registry-resource-owner" {
  principal   = "User:${confluent_service_account.gdapp-ksql.id}"
...
resource "confluent_ksql_cluster" "demo-ksql" {
  ...
  credential_identity {
    id = confluent_service_account.gdapp-ksql.id
  }
  environment {
    id = confluent_environment.demo.id
  }
  depends_on = [
    confluent_role_binding.gdapp-ksql-kafka-cluster-admin,
    confluent_role_binding.gdapp-ksql-schema-registry-resource-owner,
